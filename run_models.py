...
parser.add_argument('--raw_num', type=int, default=10000, help='the number of data points in the raw data. ')
...
# parser.add_argument('--select_interval', type=int, default=10000, help='Interval for prediction')  
# parser.add_argument('--select_scope', type=int, default=100, help='Scope for prediction')

parser.add_argument('--select_train', action='store_true', help='Select training data for prediction')
...
if __name__ == '__main__':
    ...
    def train_epoch(epo, mode="train", select=None, total_dt=None):
        ...
        if mode == "train":
            ...
            model.train() 
            combined_loader = train_combined_loader
            combined_loader.set_select(select, total_dt=total_dt)
        elif mode == "test": 
            model.eval() 
            combined_loader = test_combined_loader 
            combined_loader.set_select(select) 
        elif mode ==  "select_test": 
            model.eval() 
            combined_loader = test_combined_loader 
            combined_loader.set_select(select, total_dt=total_dt)
        elif mode == "select": 
        ...
        if (mode == "select_train") or (mode == "select_test"):
            select_mse_list = {dt: [] for dt in total_dt} 
        ...
        for itr, raw_batch in enumerate(tqdm(combined_loader)): 
            ...
            if (mode == "select_train") or (mode == "select_test"): 
                t_weights = torch.tensor(total_dt, device=device) / args.extrap_scope 
                t_weights = torch.repeat_interleave(t_weights, repeats=args.ode_forecast_frequencies[args.num_levels-2]) 
                # t_weights = t_weights.unsqueeze(0).repeat(y_prediction[f'level_{args.num_levels-1}'].shape[0], 1) # [batch_size * num_atoms, #pred_timepoints] 
                t_weights = t_weights.unsqueeze(1) 
                # set temporal weights 
                temporal_weights[f'level_{args.num_levels-1}'] = t_weights 

                # calculate mse losses separately for each time slot 
                with torch.no_grad(): 
                    y_val = y_value[f'level_{args.num_levels-1}'] 
                    y_pred = y_prediction[f'level_{args.num_levels-1}'] 
                    select_scope = args.ode_forecast_frequencies[args.num_levels-2] 
                    for j, dt in enumerate(total_dt): 
                        y_val_select = y_val[:, j*select_scope:(j+1)*select_scope, :] 
                        y_pred_select = y_pred[:, j*select_scope:(j+1)*select_scope, :]
                        select_mse_list[dt].append((y_val_select - y_pred_select).pow(2).mean().detach().cpu().item())  
        ...
        message_train = []
        message_train.append('Epoch {:04d} [{} {} seq (cond on sampled tp)] | Backprop Loss {:.6f} |'.format(epo, mode, (select if (select is not None) else ""), np.mean(loss_list))) 
        for level in range(args.num_levels): 
        ...
        # report mse for selected time slots 
        if (mode == "select_train") or (mode == "select_test"): 
            message_train.append("MSE profiling for selected time slots: ") 
            for dt in total_dt: 
                message_train.append(f"Time slot {dt}: MSE {np.mean(select_mse_list[dt])}") 
    ...
    # total_dt = np.arange(args.select_interval, args.extrap_scope, args.select_interval)


    for epo in range(1, args.niters + 1):
        if (args.select_train is True): 
            message_select_train, lookahead_message_select_train, kl_coef = train_epoch(epo, mode="select_train", select=None, total_dt=total_dt)
            fprint(message_select_train) 
        else: 
        ...
        with torch.no_grad(): 
            ...
            ''' 
            # improve efficiency for select testing 
            message_test_select, lookahead_message_test_select, kl_coef = train_epoch(epo, mode="select_test", select=None, total_dt=total_dt) 
            fprint(message_test_select) 
