...
class DiffeqSolver(nn.Module):
    ...
    def forward(self, first_point, time_steps_to_predict, graph, edge_idx_latest_observed, x_coord_latest_observed, backwards = False):
        '''

        :param first_point: 【n_sample,b*n_ball,d]
        :param time_steps_to_predict: [t]
        :param graph: [2, num_edge] (p.s. this is misleading. It's actually a flattened adj matrix)
        :param backwards:
        :return:
        '''

        assert (graph is None)
        assert (edge_idx_latest_observed is None) 

        #whether to padding 0 to the time series
        ispadding = False
        
        if time_steps_to_predict[0] != 0:
            print("splitting up the time series, and do reverse prediction on the time steps < 0. ")
            ispadding = True
            # time_steps_to_predict = torch.cat((torch.zeros(1,device=time_steps_to_predict.device),time_steps_to_predict))
            # find the position of 0 in time_steps_to_predict 
            time_step_0_idx = torch.where(time_steps_to_predict == 0)[0] 
            # get the inverse of time_steps_to_predict prior to time_step_0_idx 
            time_steps_to_predict_lookahead = time_steps_to_predict[:time_step_0_idx+1]
            time_steps_to_predict_lookahead_rev = time_steps_to_predict_lookahead.flip(0)
            time_steps_to_predict = time_steps_to_predict[time_step_0_idx:] 
        ...
        ''' We are no longer needing this, because `set_graph` is only available and necessary in `GNN()`
        ...
        self.ode_func.set_graph(None, x_coord_latest_observed)
        ...
        if ispadding: 
            pred_y_lookahead_rev = odeint(self.ode_func, first_point_augumented, time_steps_to_predict_lookahead_rev,
                rtol=self.odeint_rtol, atol=self.odeint_atol, method = self.ode_method) #[time_length, n_sample*b,n_ball, d]
            print(first_point_augumented)
            print(pred_y[0]) 
            pred_y_lookahead = pred_y_lookahead_rev.flip(0) 
            print(pred_y_lookahead[-1])
            pred_y = torch.cat((pred_y_lookahead[:-1], pred_y), dim=0) # drop the redundant 0 time step 
            # restore the time_steps_to_predict 
            time_steps_to_predict = torch.cat((time_steps_to_predict_lookahead[:-1], time_steps_to_predict), dim=0)
            assert(len(pred_y) == len(time_steps_to_predict)) 
            print(len(pred_y), len(time_steps_to_predict)) 
        ...
        '''
        # disabling padding for non-zero initialization for now 
        if ispadding:
            # pred_y = pred_y[1:,:,:,:]
            # time_steps_to_predict = time_steps_to_predict[1:]
            # inspect the time point at timepoint 0 
            print(first_point_augumented) 
            print(pred_y[time_step_0_idx, :, :, :])
            print(pred_y[0, :, :, :]) 
        ''' 
...
class GraphODEFunc(nn.Module):
    ...
    def set_graph(self, edge_idx_latest_observed, x_coord_latest_observed): 
        # print(self.nfe)
        # assert(self.ode_func_net.edge_idx_latest_observed is None)
        # assert(self.ode_func_net.x_coord_latest_observed is None)

        # self.ode_func_net.edge_idx_latest_observed = edge_idx_latest_observed
        assert (edge_idx_latest_observed is None) 
        self.ode_func_net.x_coord_latest_observed = x_coord_latest_observed
