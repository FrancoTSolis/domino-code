...
def get_lj_atom_numbers(distinguish=False):
    if distinguish: 
        atom_numbers = torch.arange(258, dtype=torch.long) 
    else: 
        atom_numbers = torch.zeros(258, dtype=torch.long) 
    return atom_numbers 

def get_tip3p_atom_numbers(distinguish=False):
    if distinguish: 
        atom_numbers = torch.arange(258*3, dtype=torch.long) 
    else: 
        # 0, 1, 1, 0, 1, 1, ... 
        atom_numbers = torch.zeros(258*3, dtype=torch.long)
        atom_numbers[1::3] = 1
        atom_numbers[2::3] = 1
    return atom_numbers


def get_tip4p_atom_numbers(distinguish=False):
    if distinguish: 
        atom_numbers = torch.arange(251*3, dtype=torch.long) 
    else: 
        # 0, 1, 1, 0, 1, 1, ... 
        atom_numbers = torch.zeros(251*3, dtype=torch.long)
        atom_numbers[1::3] = 1
        atom_numbers[2::3] = 1
    return atom_numbers
